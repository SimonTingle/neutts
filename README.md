...
FastAPI inference server for on-device text-to-speech synthesis.

Endpoints:
- `GET /health` — health check
- `POST /generate` — synthesize speech (multipart: text, ref_audio, ref_text, temperature, top_k)
- `POST /transcribe` — transcribe audio with Whisper (multipart: audio, model_id)

Requires `NEUTTS_API_KEY` secret for authentication.
